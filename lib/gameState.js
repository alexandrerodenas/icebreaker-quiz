...
async function loadOrCreateState(gameId, hostName) {
  ...
  if (hostName) {
    newState.players.push(hostName);
    newState.scores[hostName] = 0;
  }
...
export async function submitAnswer(gameId, playerName, answerIndex, answerValue) {
  const state = await loadState(gameId);
  if (!state) return { error: 'Partie introuvable' };
  ...
  if (currentQuestion.type === 'slider') {
    state.answers[playerName] = answerValue;
    correct = Math.abs(answerValue - currentQuestion.correctValue) <= currentQuestion.tolerance;
  } else {
...
function processPhaseTransitions(state) {
  if (!state || !state.isActive || state.phase !== 'playing') return false;
