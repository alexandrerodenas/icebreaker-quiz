...
export default function PlayPage() {
  ...
  }, [gameState?.currentQuestionIndex, hasAnswered]);
  ...
  const handleAnswer = async (answerIndex: number) => {
    if (hasAnswered || !gameState?.isActive) return;
    setHasAnswered(true);
    try {
  ...
  const isGameOver = !isActive || remaining <= 0 || currentQuestionIndex >= totalQuestions;
